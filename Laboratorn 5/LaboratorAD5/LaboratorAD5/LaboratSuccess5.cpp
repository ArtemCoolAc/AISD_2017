...
int main()
{
	...
	Table Table = { 0, (int)calloc(maxtablesize, sizeof(int)), (char*)calloc(maxtablesize, sizeof(int)) };
	int n;
	while (n = dialog()) {
		choice[n](&Table);
	}
...
int printtable(Table *ukaz) {
	int i = 0;
	int j = 0;
	int* index = NULL; int* index1 = NULL;
	int key;
	...
	if (ukaz->size == 0) printf_s("   Таблица пуста\n\n");
		...
		while (i < ukaz->size) {
			...
			*index = poiskelem1(ukaz, &key, copy1, index, index1);
			PK2* ptr = ukaz->tab2 + (*index);
			printf_s("   №%d, ключ 1: %d, ключ 2: %d, данные: \"%s\"\n", i, ptr1->key1, ptr->key2, ptr1->key1);
			i++;
			ptr1++;
		}
		printf_s("\n");
		...
		while (j < ukaz->size) {
			...
			*index1 = poiskelem(ukaz, &key, copy1, index, index1);
			ptr1 = ukaz->tab1+(*index1);
			printf_s("   №%d, ключ 1: %d, ключ 2: %d, данные: \"%s\"\n", j, ptr->key2, ptr1->key1, ptr->info);
			j++;
			ptr++;
		}
		printf_s("\n");
...
int findelem(Table* ukaz) {
	...
	int poiskelem(Table *ukaz, int *key, copy* copy1, int *index, int* index1) { //поиск во 2-ом пространстве, находит индекс в 1-ом
		int i = 0; 
		int m = ukaz->size - 1;
		int j, g;
		PK2* ptr = ukaz->tab2;
...
int poiskelem1(Table* ukaz, int* key, copy* copy, int* index, int* index1) { //поиск в 1-ом пространстве, находит ещё и индекс во втором
