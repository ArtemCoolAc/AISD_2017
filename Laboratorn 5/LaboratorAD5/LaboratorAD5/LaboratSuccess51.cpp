...
void dоcopy(table* ukaz, data* copy, int index) {
...
int deleteelem(table* ukaz) {
	int* key; int p; int index, index1;
	data* copy = (data *)calloc(1, sizeof(data));
	printf_s("Введите ключ, по которому нужно удалить элемент");
	key = getIntKey(&p);
	if ((index = poiskelem(ukaz, key, copy)) == -1 && (index1 = poiskelem(ukaz, key, copy)) == -1)
		printf_s("Элемент по заданному ключу не найден");
