...
int delTable(Table *);		// освобождение памяти

							// Альтернативы меню для организации диалога
const char *msgs[] = { "0. Выход", "1. Добавить элемент", "2. Найти элемент", "3. Удалить элемент", "4. Отобразить таблицу\n" };	// список альтернатив
...
int(*fptr[])(Table *) = { NULL, D_Add, D_Find, D_Delete, D_Show, delTable };
...
int main()
{
	...
	int rc;
	initTab(&table);
	while ((rc = dialog(msgs, NMsgs)) && (fptr[rc](&table)))
	{
	}
	delTable(&table);
...
int dialog(const char *msgs[], int N)
{
	...
	do {
		...
		for (i = 0; i < N; ++i) // вывод списка альтернатив
			printf_s("%s\n",msgs[i]);
		printf_s("Введите номер нужного вам действия\n");
...
const char *errmsgs[] = { "Готово!", "Таблица переполнена" };
...
int D_Find(Table *ptab)
{
	...
	if (!getInt(&key))
		return 0;
	ignore();
	printf_s("Введите номер (Если хотите увидеть все номера, введите %d) --->\n", maxsize + 1);
	if (!getInt(&rel))
		return 0;
	ignore();
	k = key;
	...
	if (rel == maxsize + 1)
	{
		...
		do {
			...
			if ((index + 1) && item)
			{
				printf_s("Ключ: %d, Номер: %d, Данные: \"%s\"\n", item->key, item->release, item->info);
				flag = 0;
			}
			else
				if (flag)
				{
					printf_s("Ошибка! Ключ не найден %d\n", k);
				}
			rel++;
	...
	else
	{
		...
		if ((index + 1) && item)
			...
			printf_s("Ошибка! Элемент по заданным условиям не найден %d\n", k);
	}
	return 1;
}
