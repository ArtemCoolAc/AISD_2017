...
void TreeSplit(Node* x, int i, Table* tbl)
	//x-родительский узел, i-позиция узла, y-разбиваемый узел
	//n-количество ключей(макс)
{
...
int InsertTree(Node** root, Table* tbl, int key, char*info) {
	//root - указ на разбиваемый корень, s-новый узел 
	Node*r = *root, *s = NULL;
	int a;
...
int DefineLeaf(Node*r) /*проверяет, является ли узел листом */
{
...
void TreeOut(Node*x) {
	int i;
	if (!DefineLeaf(x)) { //если узел - лист, то вывод
		for (i = 0; i < x->n; ++i)
			printf("   Ключ:%d-Информация:\"%s\"\n", x->element[i].key, x->element[i].info);
		return;
	}
	for (i = 0; i < x->n; ++i) { //проход по всем дочерним узлам
		TreeOut(x->ptr[i]);
		printf("   Ключ:%d-Информация:\"%s\"\n", x->element[i].key, x->element[i].info);
	}
	TreeOut(x->ptr[i]); // рекурсивный проход по всем детям детей и т.д.
	return;
}
void DelTree(Node* x) {
	int i;
...
int AddRand(Node**r, Table *tbl)        // диалоговая функция добавления рандомного элемента в дерево
{
	int k, g, i, j = 0;
	char *s = NULL;
	srand(time(NULL));
...
int main()
{
	char*msg[] = { "   0. Выход", "   1. Добавить элемент", "   2. Удалить элемент", "   3. Найти элемент", "   4. Вывести дерево на экран","   5. Найти элементы с максимальным и минимальным ключом", "   6. Форматированный вывод", "   7. Рандомное заполнение","   8. Чтение дерева из файла" }; // альтернативные действия
	...
	while (rc = Dialog(msg, Nmsg))
		fptr[rc](&root, tb);
	End(&root, tb);
	printf("   Провести таймирование? 0-Да\n   ");
	getInt(&rc);
	if (!rc)
	timemain();
	return 0;
