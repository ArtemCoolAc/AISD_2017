...
void udalil(Table* ukaz, int* index, int* index1) {
	...
	FILE* openfile(Table* ukaz) {
		...
		if (choice == 1) {
			...
			scanf_s("%*c");
			filename = getstr();
			fopen_s(&fd1, filename, "wb");
			ukaz->fd = fd1;
			printf_s("   Файл %s успешно создан", filename);
			printf_s("\n\n");
		}
		else {
	...
	FILE* opentablefile(Table* ukaz) {
		...
		fread(&(ukaz->keys2off), sizeof(int), 1, ukaz->fd); int i; int chislo[3];
		printf_s("%d %d %d  \n", ukaz->size, ukaz->keys1off, ukaz->keys2off);
		for (i = 0; i < ukaz->size; i++) {
